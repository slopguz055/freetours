    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <div class="row">
            <div>
              <label for="rol" class="form-label">Rol:</label> <!--ESto debería de ser un desplegable-->
...
const saveChanges = async () => {
  try {
    const response = await fetch(`${ipserver.value}/${editingUser.value.id}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json'
      },
...
const deleteUser = async (userId) => {
  try {
    await fetch(`${ipserver.value}/${userId}`, {
      method: 'DELETE'
    })
